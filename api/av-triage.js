...
async function callQwen(input, env) {
  const currentStatus = await status(env);
  if (!currentStatus.qwen_ready) return fallbackBrief(input);
...
async function callIbmBob(input, env) {
  const currentStatus = await status(env);
  if (currentStatus.ibm_bob_connection !== "connected") return null;
...
async function triageIncident(input, env) {
  const currentStatus = await status(env);
  if (currentStatus.ibm_bob_connection === "connected") return callIbmBob(input, env);
...
async function listRecords(req, res) {
  ...
  let records = [];
  const currentStatus = await status(process.env);

  if (currentStatus.supabase_ready) {
  ...
  return json(res, 200, { status: currentStatus, records });
}
...
module.exports = async function handler(req, res) {
  try {
    ...
    return json(res, 200, { status: currentStatus, triage, record: saved });
  } catch (error) {
